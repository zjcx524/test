...
class P2pNcclPipe:
    ...
    def __init__(self,
        ...
        logger.info("starting p2pncclpipe")
        ...
        #logger.info(f"{proxy_ip}:{proxy_port}")
        ...
        self.recv_store: Dict[str,Any] = {}  # tensor_id: torch.Tensor
        self.recv_request_id_to_tensor_ids: dict[str, set[str]] = {}# tensor_id: torch.Tensor/(addr, dtype, shape)
        ...
        logger.info(
            "P2pNcclPipe init, rank:%d, local_rank:%d, http_address:%s, "
    ...
    def _send_sync(
        ...
        if item.remote_address not in self.socks:
            self._create_connect(item.remote_address)
    ...
    def _ping(self):
        ...
        data = {
            ...
            "max_token": self.config.get_from_extra_config("max_token", ""),
            "min_token": self.config.get_from_extra_config("min_token", "")
        }
        logger.info("Ping data being sent: %s", data)
        while True:
            sock.send(msgpack.dumps(data))
            time.sleep(3)
