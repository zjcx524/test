...
class GlobalScheduler:
    ...
    def select_instance(self, token_length: int, max_tokens: int = 1, strategy: str = "round_robin", 
        ...
        # 区间匹配轮询调度（仅选择区间包含该请求的实例，有多个时轮询）
        elif strategy == "interval_match":
            return self.load_tracker.get_interval_match_instance(suitable_instances, token_length)
        else:
    ...
    async def handle_request(self):
        ...
        try:
            ...
            # 选择prefill和decode实例
            # round_robin least_loaded random slo_aware interval_match
            # 此处可修改策略
