...
def calculate_metrics(
    ...
    tokenizer: PreTrainedTokenizerBase,
    backend: str,
) -> Tuple[BenchmarkMetrics, List[int], Optional[int]]:
    ...
    # Parse goodput thresholds if provided
    goodput_ttft_threshold = None
    goodput_tpot_threshold = None
    goodput_e2el_threshold = None
    if args.goodput:
        try:
            parts = args.goodput.split(',')
            if len(parts) == 2:
                ttft_str, tpot_str = parts
                goodput_ttft_threshold = float(ttft_str)
                goodput_tpot_threshold = float(tpot_str)
            elif len(parts) == 3:
                ttft_str, tpot_str, e2el_str = parts
                goodput_ttft_threshold = float(ttft_str)
                goodput_tpot_threshold = float(tpot_str)
                goodput_e2el_threshold = float(e2el_str)
            else:
                print(f"Warning: Invalid goodput format '{args.goodput}'. Expected 'ttft,tpot' or 'ttft,tpot,e2el'.")
        except ValueError:
            print(f"Warning: Invalid goodput format '{args.goodput}'. Expected numeric values.")
    ...
    for i in range(len(outputs)):
        if outputs[i].success:
            ...
            for j in range(len(outputs[i].generated_text)):
                ...
                # Calculate TPOT
                if output_len > 1:
                    tpot_ms = (outputs[i].latency[j] - outputs[i].ttft[j]) / (output_len - 1) * 1000
                    tpots.append(tpot_ms / 1000)  # Keep in seconds for compatibility
                    
                    # Check goodput conditions
                    if goodput_ttft_threshold is not None and goodput_tpot_threshold is not None:
                        ttft_ms = outputs[i].ttft[j] * 1000
                        e2el_ms = outputs[i].latency[j] * 1000
                        
                        # Check TTFT and TPOT conditions
                        meets_ttft_tpot = ttft_ms <= goodput_ttft_threshold and tpot_ms <= goodput_tpot_threshold
                        
                        # Check E2E latency condition if specified
                        meets_e2el = True
                        if goodput_e2el_threshold is not None:
                            meets_e2el = e2el_ms <= goodput_e2el_threshold
                        
                        if meets_ttft_tpot and meets_e2el:
                            goodput_count += 1
    ...
    goodput_metrics = None
    if args.goodput and goodput_count is not None:
        goodput_req_per_s = goodput_count / dur_s  # 符合条件的请求数/持续时间
        goodput_rate_percentage = (goodput_req_per_s / metrics.request_throughput) * 100 if metrics.request_throughput > 0 else 0
        goodput_metrics = {
            "goodput_count": goodput_count,
            "goodput_req_per_s": goodput_req_per_s,
            "goodput_rate_percentage": goodput_rate_percentage
        }
    
    return metrics, output_lens, goodput_metrics
...
async def benchmark(
    ...
    metrics, output_lens, goodput_metrics = calculate_metrics(
        outputs=outputs,
    ...
    # Display goodput if specified
    if goodput_metrics is not None:
        print("{:<40} {:<10}".format("Goodput requests:", goodput_metrics["goodput_count"]))
        print("{:<40} {:<10.2f}".format("Goodput (req/s):", goodput_metrics["goodput_req_per_s"]))
        print("{:<40} {:<10.2f}%".format("Goodput rate:", goodput_metrics["goodput_rate_percentage"]))
    print("=" * 50)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Goodput thresholds in format 'ttft_ms,tpot_ms[,e2el_ms]' (e.g., '100,50' or '100,50,1000'). "
        "Count requests meeting TTFT, TPOT and optionally E2E latency thresholds.",
    )
