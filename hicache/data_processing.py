...
def sample_codearc_requests(
    ...
    for entry in dataset:
        ...
        # extract
        prompt = (
            f"The following {entry['language']} code has a bug (original status: {entry['original_status']}):\n"
            f"Problem ID: {entry['problem_id']}\n"
            f"error message: {entry['error']}\n"
            f"Code snippet (from position {entry['i1']} to {entry['i2']}):\n{entry['original_src']}\n"
            "Please provide the corrected version of the code."
        )
        completion = (
            f"{entry['change']} {entry['changed_src']} between {entry['j1']} and {entry['j2']}\n"
        )
...
def get_dataset(args, tokenizer):
    ...
    elif args.dataset_name == "aps":
        input_requests = sample_aps_requests(
            dataset_path=args.dataset_path,
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
            disable_shuffle=args.disable_shuffle,
    ...
    elif args.dataset_name == "evol":
        input_requests = sample_evol_requests(
            dataset_path=args.dataset_path,
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
            disable_shuffle=args.disable_shuffle,
    ...
    elif args.dataset_name == "code" or args.dataset_name == "arc":
        input_requests=sample_codearc_requests(
            dataset_path=args.dataset_path,
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
            disable_shuffle=args.disable_shuffle,
