...
5. **Prefill优化**：根据实例KV cache状态计算有效prompt长度进行路由

调度流程：
阶段1：查找负责当前请求长度的实例（对prefill实例使用有效长度）
...
Prefill阶段改进：
- 计算有效prompt长度 = 全部输入长度 - 实例KV cache大小
- 基于有效长度进行区间匹配和路由
- 支持通过set_instance_kv_cache()设置实例KV cache状态

关键参数：
- min_stable_interval: 最小稳定区间宽度（默认50 tokens）
- hysteresis_threshold: 滞回阈值（默认3次连续投票）
- adjustment_cooldown: 区间调整冷却时间（默认10秒）
- max_interval_change_rate: 单次最大区间变化幅度（默认100 tokens）
- prefill_transition_cost: prefill实例惩罚系数（默认0.8）
- decode_transition_cost: decode实例惩罚系数（默认0.8）
...
# 设置实例KV cache状态（用于计算有效prompt长度）
tracker.set_instance_kv_cache(prefill_instance, kv_cache_tokens=512)

# 调度请求
selected_instance = tracker.get_slo_aware_loaded_instance(
    instances=instance_list,
    input_tokens=1024,  # 全部输入长度
...
class AsyncLoadTracker:
    ...
    def __init__(self, profiler_file_path: str = None):
        ...
        # 区间匹配轮询调度相关
        self.interval_match_round_robin_index = 0  # 用于interval_match轮询调度的索引
        #----------------------------------------------------------------
    ...
    def get_interval_match_instance(self, instances: List[Instance], input_tokens: int = 0) -> Optional[Instance]:
        if not instances:
            return None
        ...
        with self.lock:
            ...
            for instance in instances:
                ...
                if instance.instance_type == "prefill":
                    # prefill实例：使用有效prompt长度（扣除KV cache）
                    check_length = self._calculate_effective_prompt_length(instance, input_tokens)
                else:
                    # decode实例或其他：使用全部输入长度
                    check_length = input_tokens
                
                # 检查请求长度是否在该实例的区间内
                if instance.min_token <= check_length <= instance.max_token:
                    matching_instances.append(instance)
            
            # 如果没有匹配的实例，返回None（fail fast）
            if not matching_instances:
                return None
            
            # 使用轮询策略从匹配的实例中选择
            index = self.interval_match_round_robin_index % len(matching_instances)
            self.interval_match_round_robin_index += 1
            selected_instance = matching_instances[index]
            
            return selected_instance


    #----------------------------------SLO感知调度------------------------------------------
    def set_instance_kv_cache(self, instance: Instance, kv_cache_tokens: int) -> None:
        """为prefill实例设置KV cache状态（用于计算有效prompt长度）
        
        Args:
            instance: 实例对象
            kv_cache_tokens: 该实例当前缓存的KV tokens数量
        """
        instance.kv_cache_tokens = max(0, kv_cache_tokens)
        print(f"Set KV cache for {instance.http_address}: {kv_cache_tokens} tokens")
    
    def get_instance_kv_cache(self, instance: Instance) -> int:
        """获取prefill实例的KV cache大小
        
        Args:
            instance: 实例对象
            
        Returns:
            该实例当前缓存的KV tokens数量（默认0）
        """
        return getattr(instance, 'kv_cache_tokens', 0)
    ...
    def get_slo_aware_loaded_instance(self, instances: List[Instance], input_tokens: int = 0, 
        ...
        实现动态实例区间自适应调整，包含滞回机制、速率限制和成本建模。
        
        改进点：
        1. 请求按 input_tokens 分类（output_tokens 仅用于估算处理时间）
        2. 对prefill实例：使用有效prompt长度（考虑KV cache）进行区间匹配
        3. 阶段1可能找到多个负责实例
        4. 跃迁成本体现为对 TTFT/TPOT 的惩罚（惩罚因子 * 延迟）
        5. 区间连续性：相邻实例区间满足 current.max_token + 1 = next.min_token
        ...
        with self.lock:
            # 对prefill实例计算有效prompt长度；对decode实例使用全部输入长度
            # effective_length用于区间匹配，input_tokens用于性能估算
            effective_prompt_length_by_instance = {}
            for instance in instances:
                if instance.instance_type == "prefill":
                    effective_length = self._calculate_effective_prompt_length(instance, input_tokens)
                else:
                    effective_length = input_tokens
                effective_prompt_length_by_instance[instance.http_address] = effective_length
            ...
            # 对prefill实例使用有效长度进行区间匹配，其他实例使用全部长度
            responsible_instances = []
            for instance in instances:
                check_length = effective_prompt_length_by_instance[instance.http_address]
                if instance.min_token <= check_length <= instance.max_token:
            ...
            if responsible_instances:
                ...
                for instance in instances:
                    check_length = effective_prompt_length_by_instance[instance.http_address]
                    if instance.max_token < check_length:
                        # 左侧实例
                        distance = check_length - instance.max_token
                        if distance < left_closest_distance:
                            left_closest_distance = distance
                            left_closest_instance = instance
                    elif instance.min_token > check_length:
                        # 右侧实例
                        distance = instance.min_token - check_length
